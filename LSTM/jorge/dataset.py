...
class MyDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir,  model_name = 'none'):  # transform = train_transform,                
        ...
        self.ids = [os.path.join(imgs_dir, x) for x in sorted(os.listdir(imgs_dir))] # direction of images
        self.msk_ids = [os.path.join(masks_dir, x) for x in sorted(os.listdir(masks_dir))] #sort both
    ...
    def __getitem__(self, index):
        ...
        #     mask = np.load(self.msk_ids[index])[[15,16,17,18],:,:] # 2016-2019, 1y interval
            ...
            img = np.load(self.ids[index])[0:18,:,:,:] # 2001-2018, 1y interval
